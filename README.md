...
**Not measured:** reclaim latency after pod eviction, scale-up latency, and throughput under the operator. The environment this was built in has no Docker daemon, so no kind cluster could be created. The harnesses are committed and ready to run; [`bench/README.md`](bench/README.md) records exactly what each one needs. No Kubernetes number was estimated or inferred from the Docker Compose figures.
